...
function chart(data,{
    ...
    Colors = ["red","green","orange","#6cf","brown","pink","purple","gray","gold"], //颜色属性，为一个向量
    xDomain,
    yDomain,
    xRange = [marginLeft, width - marginRight - 250],
    yRange = [height - marginBottom, marginTop],
    Keys = ["LoKaWai","YelizavetaDAROVSKAYA","PandalelaRinongANAKPAMG","MOONNayun","CHOEunbi","ARAIMatsuri","CHENYuxi","QUANHongchan"]
} = {}){
    //定义stack对象用于存放数据
    let series = d3.stack()
    ...
    if (xDomain === undefined) xDomain = [1,5];
    ...
    .call(xAxis.ticks(width / 300))
    .call(g => g.select(".domain").remove())
      .call(g => g.selectAll(".tick line").clone()
          .attr("y2", -height + marginTop + marginBottom)
          .attr("stroke-opacity", 0.1))
    .call(g => g.append("text")
          .attr("x", (width - 250) / 2 - 10)
          .attr("y", marginBottom)
    ...
    .call(g => g.select(".domain").remove())
      .call(g => g.selectAll(".tick line").clone()
          .attr("x2", width - marginLeft - marginRight - 250)
          .attr("stroke-opacity", 0.1))
      .call(g => g.append("text")
          .attr("x", -27)
          .attr("y", 10)
